...
start_hours = range(6, 19)  # 6..18 start (3-hr slots)
slot_duration = 3
time_slots = []
for day in days:
...
# -----------------------------
# Normalize faculty time_unavailable
# -----------------------------
for f in faculty:
...
# -----------------------------
# Build CP-SAT model & variables
# -----------------------------
model = cp_model.CpModel()
x = {}  # (sid,fid,rid,slot_label) -> BoolVar
...
for subj in course_subjects:
    ...
    if vars_for_subj:
        ...
        model.Add(0==1)  # will fail if no feasible var
...
# 4) Optional: respect max_load per faculty
for f in faculty:
    fid = f["id"]
...
if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
    ...
    print("\n❌ CP-SAT did NOT find a feasible solution. Status:", status)
...
print("\n=== Schedule ===")
if schedule:
    for s in sorted(schedule, key=lambda k: (k["faculty"], k["time"])):
        print(f"{s['course']} - {s['subject']} | {s['faculty']} | {s['room']} | {s['time']}")
else:
    print("No schedule generated by CP-SAT.")

# Show faculty allowed slots
print("\n=== Allowed slots per faculty ===")
for f in faculty:
    allowed = [t for t in time_slots if not is_time_conflict(f["unavailable_parsed"], t)]
    print(f"- {f['name']}: allowed {len(allowed)} -> {allowed[:10]}")
